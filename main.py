...
class DragCell(Button):
    ...
    def __init__(self, idx, on_long_press=None, on_drag_move=None, on_drag_end=None, **kwargs):
...
class AvatarPopup(Popup):
    ...
    def _load_av_batch(self, start, batch=8):
        ...
        for idx in range(start, end):
            ...
            # اسم الأفاتار — الضغط عليه يفتح قائمة Edit/Delete (مش pick)
            ...
            mv_btn = Button(text="Move/Copy", size_hint_y=None, height=26, font_size="8sp",
                            background_color=(0.45, 0.35, 0.7, 1))
            mv_btn.bind(on_press=lambda x, i=idx: self._open_move_copy(i))
            cell.add_widget(mv_btn)
    ...
    def _open_item_menu(self, idx):
        ...
        menu_popup = Popup(title="Options", content=content, size_hint=(0.75, 0.5))
...
class TaskDetailPopup(Popup):
    ...
    def _refresh_grid(self):
        ...
        for idx, av in enumerate(items):
            ...
            # بدون اسم) - الضغط عليه دايمًا يفتح قائمة Edit / Note / Delete
    ...
    def _open_item_menu(self, idx):
        ...
        menu_popup = Popup(title="Options", content=content, size_hint=(0.7, 0.5))
        edit_btn.bind(on_press=lambda x: (menu_popup.dismiss(), self._edit(idx)))
...
class AvatarListDetailPopup(Popup):
    ...
    def _open_item_menu(self, idx):
        ...
        menu_popup = Popup(title="Options", content=content, size_hint=(0.75, 0.45))
        edit_btn.bind(on_press=lambda x: (menu_popup.dismiss(), self._edit(idx)))
        del_btn.bind(on_press=lambda x: (menu_popup.dismiss(), self._delete(idx)))
        cancel_btn.bind(on_press=lambda x: menu_popup.dismiss())
        menu_popup.open()
