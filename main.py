...
def get_avatars_path():
    # أندرويد: استخدم مسار التطبيق الداخلي
    try:
        from android.storage import app_storage_path
        return os.path.join(app_storage_path(), "avatars.json")
    except Exception:
        pass
    # fallback للكمبيوتر أو لو مش أندرويد
    sdcard = "/sdcard/Download"
    if os.path.exists(sdcard):
        return os.path.join(sdcard, ".touchclip_avatars.json")
