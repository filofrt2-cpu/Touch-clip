...
class TaskDetailPopup(Popup):
    ...
    def _refresh_grid(self):
        ...
        for idx, av in enumerate(items):
            ...
            cell = BoxLayout(orientation="vertical", size_hint_y=None, height=150)
            ...
            float_cell = FloatLayout(size_hint_y=None, height=100)
            img_widget.pos_hint = {"x": 0, "y": 0}
            ...
            counter_btn = Button(text=str(count), size_hint=(None, None), size=(34, 30),
                                 pos_hint={"x": 0, "top": 1},
                                 background_color=(0.1, 0.45, 0.85, 0.85), font_size="13sp")
            counter_btn.bind(on_press=lambda x, i=idx: self._increment_counter(i))

            float_cell.add_widget(img_widget)
            float_cell.add_widget(img_btn)
            float_cell.add_widget(counter_btn)
            cell.add_widget(float_cell)
    ...
    def _do_add_item(self, x):
        """يفتح نافذة لاختيار آخر صورة + اسم + لينك ويضيفها لعناصر المهمة."""
        imgs = engine.get_new_images()
        if not imgs:
    ...
    def _open_item_menu(self, idx):
        ...
        reset_btn  = Button(text="↺ Reset Counter", background_color=(0.4, 0.4, 0.45, 1))
        ...
        reset_btn.bind(on_press=lambda x: (menu_popup.dismiss(), self._reset_counter(idx)))
        del_btn.bind(on_press=lambda x: (menu_popup.dismiss(), self._delete(idx)))
        cancel_btn.bind(on_press=lambda x: menu_popup.dismiss())
        menu_popup.open()
    ...
    def _reset_counter(self, idx):
        self._items()[idx]["count"] = 0
        self._persist()
        self._refresh_grid()
