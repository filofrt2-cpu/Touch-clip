...
import os, time, base64, copy, io, re, json, zipfile, shutil
...
def import_avatars():
    try:
        # دور على الـ zip في Download
        zips = [f for f in os.listdir("/sdcard/Download") if f.endswith(".zip") and "touchclip" in f.lower()]
        if not zips:
            return "No touchclip zip found in /sdcard/Download"
        zip_path = "/sdcard/Download/" + sorted(zips)[-1]
        ensure_avatars_dir()
        # فك الـ zip
        with zipfile.ZipFile(zip_path, "r") as zf:
            zf.extractall(AVATARS_DIR)
        # حمّل الـ JSON اللي اتفك
        loaded = load_avatars()
        if not loaded:
            return "Zip extracted but avatars.json not found or empty"
        # صلّح المسارات تشاور على المجلد الجديد
        for av in loaded:
            if "path" in av:
...
def delete_avatar(idx):
    try:
        av = AVATARS[idx]
        # احذف ملف الصورة لو موجود
        img_path = av.get("path", "")
        if img_path and os.path.exists(img_path):
            os.remove(img_path)
        AVATARS.pop(idx)
    ...
    def _refresh_grid(self):
        ...
        for idx, av in enumerate(AVATARS):
            ...
            try:
                ...
                img_widget = Image(texture=core_img.texture, size_hint_y=0.7)
            except Exception:
                img_widget = Label(text="?", size_hint_y=0.7)
            cell.add_widget(img_widget)
            btn = Button(text=av.get("name", "No Name"), size_hint_y=0.15)
            btn.bind(on_press=lambda x, i=idx: self._pick(i))
            cell.add_widget(btn)
            del_btn = Button(text="✕ Delete", size_hint_y=0.15, background_color=(0.8, 0.2, 0.2, 1))
            del_btn.bind(on_press=lambda x, i=idx: self._delete(i))
...
class UI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        row1c.add_widget(Button(text="Export", on_press=self.do_export_avatars, font_size="11sp"))
        row1c.add_widget(Button(text="Import", on_press=self.do_import_avatars, font_size="11sp"))
    ...
    def _set_avatar(self, avatar):
        ...
        if img_path and os.path.exists(img_path):
            try:
                engine.author_avatar_b64 = base64.b64encode(open(img_path, "rb").read()).decode("utf-8")
            except Exception:
    ...
    def do_grab_avatar(self, x):
        ...
        def on_confirm(name, b64):
            try:
                ...
                return
            new_avatar = {"name": name, "path": img_path, "ext": ext}
