...
# =========================
# ZIP FILE PICKER POPUP
# =========================
class ZipPickerPopup(Popup):
    def __init__(self, on_select, **kwargs):
        super().__init__(**kwargs)
        self.title = "اختار ملف ZIP"
        ...
        btn_row = BoxLayout(size_hint_y=None, height=48)
        refresh_btn = Button(text="🔄 تحديث")
        refresh_btn.bind(on_press=lambda x: self._load())
    ...
    def _load(self):
        ...
        if not zips:
            self.list_box.add_widget(
                Label(text="مفيش ملفات ZIP في الجهاز", size_hint_y=None, height=48)
            )
            return
        for path in zips:
            try:
            ...
            btn = Button(text=label, size_hint_y=None, height=52,
                         text_size=(None, None), halign="right",
                         font_size="12sp")
            btn.bind(on_press=lambda x, p=path: self._pick(p))
