...
class AvatarPopup(Popup):
    ...
    def _open_item_menu(self, idx):
        """زرار ⋮ العلوي: ترتيب (Up/Down) + Edit Avatar / Move to Scroll / Delete Avatar
        ...
        order_row = BoxLayout(orientation="horizontal", spacing=6, size_hint_y=0.14)
    ...
    def _reorder(self, idx, direction):
        ...
        if 0 <= new_idx < len(AVATARS):
            AVATARS[idx], AVATARS[new_idx] = AVATARS[new_idx], AVATARS[idx]
            save_avatars()
            self._refresh_grid()
    ...
    def _delete(self, idx):
        ...
        def confirm_delete(instance):
            confirm_popup.dismiss()
            img_path = av.get("path", "")
            scroll_paths = {s.get("path", "") for lst in SCROLL_LISTS for s in lst.get("avatars", [])}
...
class MyApp(App):
    ...
    def on_start(self):
        global AVATARS, SCROLL_AVATARS, SCROLL_LISTS, SCROLL_ACTIVE_LIST, TASKS
