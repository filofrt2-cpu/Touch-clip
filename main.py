...
def import_avatars():
    try:
        ...
        zip_path = "/sdcard/Download/" + sorted(zips, key=lambda z: os.path.getmtime("/sdcard/Download/" + z))[-1]
        ensure_avatars_dir()
        with zipfile.ZipFile(zip_path, "r") as zf:
            zf.extractall(AVATARS_DIR)
        loaded = load_avatars()
        # fix paths to point to new location
        for av in loaded:
            if "path" in av:
...
""")
                ...
                if engine.quote_text or engine.quote_link or engine.quote_images:
                    ...
                    # header: أفاتار + اسم صاحب البوست المُقتبَس
                    if engine.quote_avatar_b64 or engine.quote_author_name:
                        f.write('<div class="quote-header">\n')
                        if engine.quote_avatar_b64:
                            f.write(f'<img class="quote-avatar" src="data:image/{engine.quote_avatar_ext};base64,{engine.quote_avatar_b64}">\n')
