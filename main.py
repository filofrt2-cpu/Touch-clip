...
def fix_ar(text):
    """يشكّل ويرتب النص العربي عشان يظهر متصل وبالاتجاه الصحيح في Kivy.
    النصوص اللي مفيهاش عربي بترجع زي ما هي بدون أي تغيير."""
    if not text or not isinstance(text, str) or not HAS_ARABIC_SHAPING:
        return text
...
def _patch_widget_arabic_support():
    """بنخلي Label و Button و TextInput يشكّلوا أي نص عربي تلقائيًا
    وقت ما الـ text بتاعهم يتغيّر - بدل ما نعدّل كل سطر فيه نص عربي
    في الكود (خطر إننا ننسى مكان). الحل ده بيغطي كل حالة حالية ومستقبلية.

    بنعمل override لـ __init__ بتاع كل widget عشان نضمن إن كل instance
    (حتى اللي يتعمل بعدين في أي مكان في الكود) يتربط تلقائيًا،
    لأن bind على مستوى الكلاس نفسه مش API صحيح في Kivy - لازم instance."""
    from kivy.uix.label import Label as _Label
    from kivy.uix.textinput import TextInput as _TextInput

    def _make_text_fixer(attr_name, flag_name):
        def _on_text_change(instance, value):
            if getattr(instance, flag_name, False):
                return
            fixed = fix_ar(value)
            if fixed != value:
                setattr(instance, flag_name, True)
                setattr(instance, attr_name, fixed)
                setattr(instance, flag_name, False)
        return _on_text_change

    _on_label_text = _make_text_fixer("text", "_fixing_ar")
    _on_hint_text  = _make_text_fixer("hint_text", "_fixing_ar_hint")

    _orig_label_init = _Label.__init__
    def _patched_label_init(self, **kwargs):
        if "text" in kwargs:
            kwargs["text"] = fix_ar(kwargs["text"])
        _orig_label_init(self, **kwargs)
        self.bind(text=_on_label_text)
    _Label.__init__ = _patched_label_init

    _orig_textinput_init = _TextInput.__init__
    def _patched_textinput_init(self, **kwargs):
        if "hint_text" in kwargs:
            kwargs["hint_text"] = fix_ar(kwargs["hint_text"])
        _orig_textinput_init(self, **kwargs)
        self.bind(hint_text=_on_hint_text)
    _TextInput.__init__ = _patched_textinput_init

if HAS_ARABIC_SHAPING:
    try:
        _patch_widget_arabic_support()
    except Exception:
        pass

# =========================
# دعم اللغة العربية + الإيموجي
# المشكلة: خط Kivy الافتراضي (Roboto) معندوش حروف عربي ولا إيموجي
# فبيظهروا كمربعات (tofu boxes). الحل: نسجل خط بديل بيدعمهم
# ونخليه الخط الافتراضي بدل Roboto، عشان مفيش KV هنا بنعمله من الكود.
# =========================
def _setup_fonts():
    """نسجل خط واحد بيدعم العربي والإنجليزي معاً باسم "Roboto" (الخط الافتراضي).
    المشكلة القديمة: NotoSansArabic-Regular.ttf كان بيستبدل "Roboto" بالكامل،
    وهو خط عربي فقط مش فيه حروف لاتينية فالإنجليزي ظهر كمربعات (tofu boxes).
    الحل: نفضّل خط "Noto Sans" العادي (مش Arabic-only) اللي بيغطي اللاتيني
    والعربي مع بعض. لو غير متاح، نستخدم DejaVuSans (لاتيني فقط، يضمن
    الإنجليزي يفضل شغال حتى لو العربي يحتاج تشكيل بديل من مكان تاني)."""
    candidates_universal = [
        # خطوط تدعم العربي + اللاتيني مع بعض في نفس الملف
        "/system/fonts/NotoSansArabicUI-Regular.ttf",
        "/system/fonts/NotoNaskhArabicUI-Regular.ttf",
        "/system/fonts/Roboto-Regular.ttf",  # الخط الأصلي على أندرويد (لاتيني)
        "C:/Windows/Fonts/seguiemj.ttf",
        "C:/Windows/Fonts/arial.ttf",
        "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
    ]
    candidates_arabic_only = [
        # دول خطوط عربي فقط - نسجلهم باسم مختلف، مش "Roboto"،
        # عشان مانكسرش عرض الإنجليزي
    ...
    universal_font   = next((p for p in candidates_universal   if os.path.exists(p)), None)
    arabic_only_font = next((p for p in candidates_arabic_only if os.path.exists(p)), None)
    emoji_font       = next((p for p in candidates_emoji       if os.path.exists(p)), None)

    registered_default = False
    if universal_font:
        try:
            LabelBase.register(name="Roboto", fn_regular=universal_font)
            registered_default = True
        except Exception:
            pass

    if not registered_default and arabic_only_font:
        # آخر حل: نسجل خط العربي فقط كافتراضي عشان العربي يفضل شغال
        try:
            LabelBase.register(name="Roboto", fn_regular=arabic_only_font)
            registered_default = True
        except Exception:
            pass
    ...
    if arabic_only_font:
        try:
            LabelBase.register(name="ArabicFont", fn_regular=arabic_only_font)
        except Exception:
            pass

    if emoji_font:
        try:
            LabelBase.register(name="NotoEmoji", fn_regular=emoji_font)
        except Exception:
            pass
...
def grab_article(url):
    ...
    if not HAS_ARTICLE_DEPS:
        return {"error": "مكتبات requests / beautifulsoup4 غير مثبّتة.\nأضفهم في buildozer.spec ضمن requirements."}

    url = article_clean_url(url)
    if not re.match(r"^https?://", url):
        return {"error": "الرابط غير صحيح. يجب أن يبدأ بـ http:// أو https://"}
    ...
    try:
        ...
        try:
            ...
            if ctype and "text/html" not in ctype and "application/xhtml" not in ctype:
                return {"error": f"الرابط لا يشير إلى صفحة HTML (Content-Type: {ctype})."}
            if clen and int(clen) > ARTICLE_MAX_BYTES:
                return {"error": "حجم الصفحة كبير جداً (أكبر من 8MB)."}
        except Exception:
        ...
        if status in (403, 503) and ("cloudflare" in body or "cf-browser-verification" in body or "attention required" in body):
            return {"error": "هذا الموقع محمي بواسطة Cloudflare ولا يمكن الوصول إليه مباشرة.\nجرب فتح الرابط في المتصفح أولاً."}
        return {"error": f"فشل الطلب (HTTP {status}).\n{e}"}
    except requests.exceptions.Timeout:
        return {"error": "انتهت مهلة الاتصال بالموقع (Timeout). جرب مرة أخرى."}
    except requests.exceptions.ConnectionError as e:
        return {"error": f"فشل الاتصال بالموقع.\n{e}"}
    except Exception as e:
    ...
    elements = soup.find_all(["p", "img", "iframe"])
    for el in elements:
        if el.name == "p":
...
def article_embed_images_offline(data, max_px=900, quality=60, progress_cb=None, max_workers=4):
    ...
    urls = list(dict.fromkeys(urls))  # إزالة التكرار مع الحفاظ على الترتيب
    if not urls:
        return data
...
def article_to_html(data):
    ...
    body_lines = []
    for raw_line in data["markdown"].split("\n"):
        line = raw_line.strip()
        if not line:
            continue
        if line.startswith("# "):
        ...
        text_for_links = line[2:-2] if is_bold_line else line
...
<body>
<h1>{title}</h1>
<div class="meta">✍️ {author} &nbsp;|&nbsp; 📅 {date} &nbsp;|&nbsp; <a href="{_html.escape(source)}" target="_blank">المصدر</a></div>
{body_html}
</body>
</html>"""
...
class AvatarPopup(Popup):
    ...
    def _refresh_grid(self):
        ...
        for idx, av in enumerate(AVATARS):
            ...
            # نستخدم FloatLayout عشان نركب: الصورة + زرار الاختيار (تحتها)
            # + زرار "⋮" menu صغير في الزاوية العليا (Edit/Delete) بدل الزرار
            # الأحمر القديم اللي كان تحت الصورة
            float_cell = FloatLayout(size_hint_y=0.65)
            ...
            menu_btn = Button(text="⋮", size_hint=(None, None), size=(36, 36),
                              pos_hint={"right": 1, "top": 1},
                              background_color=(0, 0, 0, 0.55), font_size="18sp")
            ...
            # زرار الاختيار مع قطع الاسم الطويل
            av_name = trim_name(av.get("name", "No Name"))
            btn_select = Button(text=av_name, size_hint_y=0.18,
                                text_size=(None, None),
                                halign="center", valign="middle",
                                shorten=True, shorten_from="right",
                                font_size="12sp")
            btn_select.bind(size=lambda inst, val: setattr(inst, 'text_size', (val[0] - 8, None)))
    ...
    def _open_item_menu(self, idx):
        ...
        menu_popup = Popup(title="خيارات", content=content, size_hint=(0.7, 0.4))
    ...
    def _delete(self, idx):
        ...
        content.add_widget(Label(text=f'حذف "{av.get("name","")}" ؟', font_size="15sp"))
        btn_row = BoxLayout(size_hint_y=None, height=44, spacing=8)
        yes_btn = Button(text="حذف", background_color=(0.85, 0.2, 0.2, 1))
        no_btn  = Button(text="إلغاء")
        btn_row.add_widget(yes_btn)
        btn_row.add_widget(no_btn)
        content.add_widget(btn_row)
        confirm_popup = Popup(title="تأكيد الحذف", content=content,
                              size_hint=(0.75, 0.3))
...
class NamePopup(Popup):
    ...
    def __init__(self, b64_data, ext, on_confirm, **kwargs):
        ...
        self.name_input = TextInput(hint_text="اسم صاحب البوست", multiline=False, size_hint_y=0.12)
        root.add_widget(self.name_input)

        self.link_input = TextInput(hint_text="لينك (مطلوب لـ Scroll / Both)", multiline=False, size_hint_y=0.12)
        root.add_widget(self.link_input)

        hint_lbl = Label(
            text="اختر الوجهة:  Avatar = اسم وصورة فقط  |  Scroll = اسم وصورة ولينك  |  Both = الاتنين",
    ...
    def _confirm(self, destination):
        name = self.name_input.text.strip()
        if not name:
            self.name_input.hint_text = "⚠ لازم تكتب اسم"
            return
        link = self.link_input.text.strip()
        if destination in ("scroll", "both") and not link:
            self.link_input.hint_text = "⚠ لازم لينك لـ Scroll/Both"
            return
...
class ScrollAvatarPopup(Popup):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.title = "Scroll — اضغط الصورة لفتح اللينك"
        self.size_hint = (0.9, 0.9)
    ...
    def _refresh_grid(self):
        ...
        if not SCROLL_AVATARS:
            self.grid_container.add_widget(
                Label(text="لا يوجد أفاتار بلينكات.\nأضف لينك عند الضغط على ADD.")
            )
            return
        ...
        for idx, av in enumerate(SCROLL_AVATARS):
            ...
            # اسم الأفاتار — مقطوع بنقاط
            av_name = trim_name(av.get("name", "No Name"))
            lbl_name = Label(text=av_name, size_hint_y=0.18,
    ...
    def _delete(self, idx):
        ...
        content = BoxLayout(orientation="vertical", spacing=10, padding=12)
        content.add_widget(Label(text=f'حذف "{av.get("name","")}" من Scroll ؟', font_size="15sp"))
        btn_row = BoxLayout(size_hint_y=None, height=44, spacing=8)
        yes_btn = Button(text="حذف", background_color=(0.85, 0.2, 0.2, 1))
        no_btn  = Button(text="إلغاء")
        btn_row.add_widget(yes_btn)
...
class ArticlePopup(Popup):
    def __init__(self, on_saved, **kwargs):
        ...
        root.add_widget(Label(text="ضع رابط المقال:", size_hint_y=0.08, font_size="13sp"))
        ...
        self.preview = Label(text="(لا يوجد مقال محمّل بعد)", halign="left", valign="top",
    ...
    def _do_grab(self, x):
        ...
        if not url:
            self.preview.text = "⚠️ ادخل رابط المقال أولاً."
            return
        if not HAS_ARTICLE_DEPS:
            self.preview.text = ("❌ مكتبات requests / beautifulsoup4 غير مثبّتة في هذا البناء.\n"
                                  "أضفهم إلى requirements في buildozer.spec:\n"
                                  "requests,beautifulsoup4,certifi,urllib3,charset-normalizer,idna")
            return

        self.busy = True
        self.grab_btn.text = "...جاري الجلب"
        self.grab_btn.disabled = True
        self.preview.text = "⏳ جاري جلب المقال..."
    ...
    def _do_save(self, x):
        if not self.result:
            self.preview.text = "⚠️ اجلب مقالاً أولاً قبل الحفظ (اضغط GRAB)."
            return

        content = BoxLayout(orientation="vertical", spacing=10, padding=12)
        content.add_widget(Label(text="اختر صيغة الحفظ:", font_size="14sp"))
        ...
        content.add_widget(btn_row)
        cancel_btn = Button(text="إلغاء", size_hint_y=None, height=40)
        content.add_widget(cancel_btn)

        choice_popup = Popup(title="حفظ المقال", content=content, size_hint=(0.8, 0.32))
    ...
    def _save_as(self, choice_popup, fmt):
        ...
        self.preview.text = "⏳ جاري تحميل وضغط صور المقال (أوفلاين)...\n0%"
        self.grab_btn.disabled = True
        ...
        def progress_cb(done, total):
            ...
            Clock.schedule_once(
                lambda dt: setattr(self.preview, "text",
                                    f"⏳ جاري تحميل وضغط صور المقال (أوفلاين)...\n{done}/{total} ({pct}%)"),
                0
            )

        def worker():
            try:
                offline_res = article_embed_images_offline(res, progress_cb=progress_cb)
            except Exception as e:
                offline_res = res
                Clock.schedule_once(lambda dt: setattr(self.preview, "text",
                                     "⚠️ فشل تحميل بعض الصور، سيتم الحفظ بدون أوفلاين كامل: " + str(e)), 0)
    ...
    def _finish_save(self, res, fmt):
        ...
        try:
            if fmt == "md":
                out_text = res["markdown"]
            else:
                out_text = article_to_html(res)
            with open(path, "w", encoding="utf-8") as f:
                f.write(out_text)
        except Exception as e:
            self.preview.text = "❌ فشل الحفظ: " + str(e)
            return

        self.preview.markup = False
        self.preview.text = "✓ تم الحفظ في:\n" + path
        if self.on_saved:
            self.on_saved(path)
...
class UI(BoxLayout):
    ...
    def _on_article_saved(self, path):
        engine.saved = True
        self.status.text = "✓ تم حفظ المقال: " + path
        self.update_status()
    ...
    def do_grab_avatar(self, x):
        ...
        def on_confirm(name, b64, link="", destination="avatar", _ext=captured_ext):
            ...
            self.status.text = "✓ Saved to: " + " + ".join(saved_to) if saved_to else "❌ لم يتم الحفظ"
