...
# arabic_reshaper لإصلاح العربي في الـ status
try:
...
try:
    ...
    for fp in arabic_font_paths:
        if os.path.exists(fp):
            LabelBase.register(name="Arabic", fn_regular=fp)
            ARABIC_FONT = "Arabic"
            break
except Exception:
    ARABIC_FONT = "Roboto"

# مسار حفظ الأفاتار على الجهاز
# مسار حفظ الأفاتار - يشتغل على أندرويد وكمبيوتر
def get_avatars_path():
    # أندرويد: /sdcard/Download
    android_path = "/sdcard/Download/.touchclip_avatars.json"
    if os.path.exists("/sdcard/Download"):
        return android_path
    return os.path.join(os.path.expanduser("~"), ".touchclip_avatars.json")
...
# =========================
# تحميل الأفاتار المحفوظة من الجهاز
def load_avatars():
    try:
...
class NamePopup(Popup):
    def __init__(self, b64_data, ext, on_confirm, **kwargs):
        ...
        self.name_input = TextInput(hint_text="اسم صاحب البوست", multiline=False, size_hint_y=0.15, font_name=ARABIC_FONT)
...
class UI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.status = Label(text="", size_hint_y=0.7, halign="center", valign="middle", font_size="13sp", font_name=ARABIC_FONT)
        self.status.bind(size=self._update_status_text_size)
        self.add_widget(self.status)

        Clock.schedule_once(lambda dt: Clock.schedule_interval(self._safe_capture, 1.2), 2)
        self.update_status()
    ...
    def do_save(self, x):
        ...
        first_line = engine.post.split("\n")[0].strip()[:40]
        clean = "".join(c for c in first_line if c.isalnum() or c in " _").strip().replace(" ", "_")
        path = "/sdcard/Download/" + (clean or "post") + ".html"
        post_title = engine.post.split("\n")[0].strip()[:60]
...
""" + favicon_tag + f"<title>{post_title}</title>\n" + """
<style>
  body {
...
function toggleReplies(btn) {
  ...
  btn.classList.toggle('collapsed');
    ...
    def update_status(self):
        ...
        lines = [
            f"POST:{b(engine.post)}  LINK:{b(engine.post_link)}  IMAGE:{len(engine.post_images)}",
            "",
            f"QUOTE:{b(engine.quote_text)}  Q.LINK:{b(engine.quote_link)}  Q.IMG:{len(engine.quote_images)}",
            "",
            f"COMMENTS:{total_comments}  LAST:{cur}  ATTACH:{attach_count}",
            "",
            f"AVATAR: {fix_arabic(engine.author_name) if engine.author_name else '-'}",
            "─" * 30,
