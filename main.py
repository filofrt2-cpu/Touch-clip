...
class DragCell(Button):
    ...
    def on_touch_down(self, touch):
        ...
        # في وضع Arrange بقينا بنستخدم on_press عادي (يفتح ديالوج تحديد
        # المكان بدل السحب)، فمفيش داعي نجدول long-press/drag هنا خالص.
        if handled and self.collide_point(*touch.pos) and self.on_long_press and not self.arrange_mode:
...
# =========================
# REPOSITION DIALOG: بدل السحب - في وضع Arrange الضغط على أي أفاتار
# بيفتح ديالوج يوريك مكانه الحالي (صف/عمود) ويسيبك تكتب المكان الجديد
# اللي عاوزه، وهو بينقل العنصر هناك في الليستة.
# =========================
def open_reposition_popup(items, idx, cols, on_moved):
    total = len(items)
    cur_row = idx // cols + 1
    cur_col = idx % cols + 1

    content = BoxLayout(orientation="vertical", spacing=8, padding=12)
    content.add_widget(Label(text=f"Current position: Row {cur_row}, Column {cur_col}",
                              size_hint_y=None, height=30, font_size="13sp"))

    row_box = BoxLayout(size_hint_y=None, height=44, spacing=6)
    row_box.add_widget(Label(text="New Row:", size_hint_x=0.4))
    row_input = TextInput(text=str(cur_row), multiline=False, input_filter="int", size_hint_x=0.6)
    row_box.add_widget(row_input)
    content.add_widget(row_box)

    col_box = BoxLayout(size_hint_y=None, height=44, spacing=6)
    col_box.add_widget(Label(text=f"New Column (1-{cols}):", size_hint_x=0.6))
    col_input = TextInput(text=str(cur_col), multiline=False, input_filter="int", size_hint_x=0.4)
    col_box.add_widget(col_input)
    content.add_widget(col_box)

    btn_row = BoxLayout(size_hint_y=None, height=44, spacing=6)
    move_btn = Button(text="Move", background_color=(0.2, 0.6, 0.3, 1))
    cancel_btn = Button(text="Cancel")
    btn_row.add_widget(move_btn)
    btn_row.add_widget(cancel_btn)
    content.add_widget(btn_row)

    popup = Popup(title="Move Avatar", content=content, size_hint=(0.8, 0.5))

    def _do_move(x):
        try:
            r = int(row_input.text) if row_input.text else cur_row
        except ValueError:
            r = cur_row
        try:
            c = int(col_input.text) if col_input.text else cur_col
        except ValueError:
            c = cur_col
        r = max(1, r)
        c = max(1, min(cols, c))
        new_idx = (r - 1) * cols + (c - 1)
    ...
    move_btn.bind(on_press=_do_move)
...
class AvatarPopup(Popup):
    ...
    def _load_av_batch(self, start, batch=8):
        ...
        for idx in range(start, end):
            ...
            if self._arrange_mode:
                img_btn.bind(on_press=lambda x, i=idx: self._open_reposition(i))
            else:
    ...
    def _open_reposition(self, idx):
        open_reposition_popup(AVATARS, idx, 5, lambda: (save_avatars(), self._refresh_grid()))
...
class TaskDetailPopup(Popup):
    ...
    def _open_reposition(self, idx):
        open_reposition_popup(self._items(), idx, 4, lambda: (self._persist(), self._refresh_grid()))
