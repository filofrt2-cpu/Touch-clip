...
def _setup_fonts():
    candidates_arabic = [
        # خطوط أندرويد الجاهزة (موجودة على كل الأجهزة تقريباً)
        "/system/fonts/NotoSansArabic-Regular.ttf",
        "/system/fonts/NotoNaskhArabic-Regular.ttf",
        "/system/fonts/NotoSansDevanagari-Regular.ttf",
        # fallback لو بتجرب على ويندوز/لينكس/ماك أثناء التطوير
    ...
    arabic_font = next((p for p in candidates_arabic if os.path.exists(p)), None)
    emoji_font  = next((p for p in candidates_emoji  if os.path.exists(p)), None)

    if arabic_font:
        try:
            # بنسجله باسم "Roboto" عشان ده اسم الخط الافتراضي اللي
            # كل الـ Label/Button/TextInput في التطبيق بيستخدموه لو معملنالهم font_name
...
class AvatarPopup(Popup):
    ...
    def _refresh_grid(self):
        ...
        for idx, av in enumerate(AVATARS):
            ...
            # صورة الأفاتار قابلة للضغط (تختار بالضغط على الصورة أو الزر)
            img_btn = Button(size_hint_y=0.65, background_color=(0, 0, 0, 0))
            img_btn.bind(on_press=lambda x, i=idx: self._pick(i))
            img_overlay = BoxLayout(size_hint_y=0.65)
            img_overlay.add_widget(img_widget)
            # نستخدم FloatLayout صغير لتركيب الصورة فوق الزر
            from kivy.uix.floatlayout import FloatLayout
            float_cell = FloatLayout(size_hint_y=0.65)
            img_widget.size_hint = (1, 1)
            img_widget.pos_hint = {"x": 0, "y": 0}
            img_btn.size_hint = (1, 1)
            img_btn.pos_hint = {"x": 0, "y": 0}
...
class NamePopup(Popup):
    def __init__(self, b64_data, ext, on_confirm, **kwargs):
        ...
        try:
            ...
            img_widget = Image(texture=core_img.texture, size_hint_y=0.52)
            root.add_widget(img_widget)
        except Exception:
            root.add_widget(Label(text="(preview unavailable)", size_hint_y=0.52))

        self.name_input = TextInput(hint_text="اسم صاحب البوست", multiline=False, size_hint_y=0.13)
        root.add_widget(self.name_input)

        self.link_input = TextInput(hint_text="لينك (اختياري - للـ Scroll)", multiline=False, size_hint_y=0.13)
        root.add_widget(self.link_input)

        btn_row = BoxLayout(size_hint_y=0.13)
    ...
    def _confirm(self, x):
        ...
        self.on_confirm(name, self.b64_data, link)
        self.dismiss()
        ...
        for idx, av in enumerate(SCROLL_AVATARS):
            ...
            try:
                ...
                if img_path and os.path.exists(img_path):
                    ...
                    raw = base64.b64decode(av["b64"])
                    core_img = CoreImage(io.BytesIO(raw), ext=img_ext)
                img_widget = Image(texture=core_img.texture)
            except Exception:
                img_widget = Label(text="?")
            ...
            # زرار الحذف من Scroll فقط
            btn_del = Button(text="🗑 Remove", size_hint_y=0.17,
                             background_color=(0.85, 0.2, 0.2, 1), font_size="11sp")
            btn_del.bind(on_press=lambda x, i=idx: self._delete(i))
            cell.add_widget(btn_del)
...
class UI(BoxLayout):
    ...
    def do_grab_avatar(self, x):
        ...
        def on_confirm(name, b64, link=""):
            try:
                img_path, saved_ext = save_avatar_image(b64, ext, name)
            except Exception as e:
            ...
            # لو في لينك → أضفه لـ SCROLL_AVATARS أيضاً
            if link:
