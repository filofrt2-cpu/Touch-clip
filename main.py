...
def grab_article(url):
    ...
    if article_containers:
        ...
        elements = best.find_all(["p", "h2", "h3", "h4", "img", "iframe", "ul", "ol", "blockquote"])
    else:
        elements = soup.find_all(["p", "h2", "h3", "h4", "img", "iframe", "ul", "ol", "blockquote"])
...
class ScrollAvatarPopup(Popup):
    def __init__(self, grab_avatar_cb=None, **kwargs):
        ...
        # صف أزرار التحكم العلوي
        top_row = BoxLayout(size_hint_y=None, height=46, spacing=6, padding=(4, 2))
        add_av_btn = Button(text="➕ Add Avatar", font_size="12sp",
                            background_color=(0.15, 0.6, 0.35, 1))
        add_av_btn.bind(on_press=self._do_add_avatar)
        top_row.add_widget(add_av_btn)
    ...
    def _do_add_avatar(self, x):
        """يفتح نافذة لاختيار آخر صورة + اسم + لينك ويضيفها للـ Scroll"""
        imgs = engine.get_new_images()
        if not imgs:
            # نافذة رسالة خطأ
            err = BoxLayout(orientation="vertical", spacing=8, padding=12)
            err.add_widget(Label(text="No new images found.\nSave a photo first, then try again."))
            ok_btn = Button(text="OK", size_hint_y=None, height=40)
            err.add_widget(ok_btn)
            ep = Popup(title="No Images", content=err, size_hint=(0.75, 0.35))
            ok_btn.bind(on_press=lambda i: ep.dismiss())
            ep.open()
            return
        ...
        btn_row = BoxLayout(size_hint_y=0.16, spacing=6)
        ok_btn  = Button(text="✓ Add to Scroll", background_color=(0.15, 0.6, 0.35, 1))
        can_btn = Button(text="Cancel")
        btn_row.add_widget(ok_btn)
        btn_row.add_widget(can_btn)
        root.add_widget(btn_row)

        add_popup = Popup(title="Add to Scroll", content=root, size_hint=(0.82, 0.7))
        ...
        def _confirm(inst):
            ...
            entry = {"name": name, "link": link, "path": img_path,
                     "ext": saved_ext, "b64": b64_data}
            SCROLL_AVATARS.append(entry)
            save_scroll_avatars()
            add_popup.dismiss()
            self._refresh_grid()
...
class TaskPopup(Popup):
    def __init__(self, **kwargs):
        ...
        # زرار إضافة مهمة جديدة
        add_btn = Button(text="+ Add Task", size_hint_y=None, height=44,
    ...
    def _refresh(self):
        ...
        for idx, task in enumerate(TASKS):
            ...
            check_btn = Button(
                ...
                background_color=(0.2, 0.65, 0.3, 1) if done else (0.5, 0.5, 0.5, 1)
            )
            check_btn.bind(on_press=lambda x, i=idx: self._toggle_done(i))

            lbl = Label(text=task.get("name", ""), halign="left", valign="middle",
                        font_size="13sp",
                        color=(0.5, 0.5, 0.5, 1) if done else (1, 1, 1, 1))
    ...
    def _show_add_dialog(self, x):
        ...
        ok_btn  = Button(text="Add", background_color=(0.2, 0.65, 0.3, 1))
        can_btn = Button(text="Cancel")
        btn_row.add_widget(ok_btn)
        btn_row.add_widget(can_btn)
        content.add_widget(btn_row)

        dlg = Popup(title="Add Task", content=content, size_hint=(0.8, 0.36))
