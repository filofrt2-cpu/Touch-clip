...
def import_avatars(zip_path):
    try:
        ...
        slim_legacy_avatar_blobs(SCROLL_AVATARS, None)
        save_scroll_avatars()
...
class AvatarPopup(Popup):
    ...
    def _open_item_menu(self, idx):
        """زرار ⋮ العلوي: يفتح قائمة صغيرة فيها Edit Avatar / Move to Scroll / Delete Avatar
        ...
        menu_popup = Popup(title="Options", content=content, size_hint=(0.7, 0.45))
...
class MyApp(App):
    ...
    def on_start(self):
        ...
        slim_legacy_avatar_blobs(SCROLL_AVATARS, save_scroll_avatars)
        tasks_changed = False
        for task in TASKS:
